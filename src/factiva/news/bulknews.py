...
class BulkNewsBase(object):
    """
        ...
        api_user : str or APIKeyUser
            String containing the 32-character long APi Key. If not provided, the constructor will try to obtain its value from the FACTIVA_APIKEY environment variable.
        request_userinfo : boolean, optional (Default: False)
            Indicates if user data has to be pulled from the server. This operation fills account detail properties along with maximum, used and remaining values. It may take several seconds to complete.
...
class BulkNewsJob(object):
    ...
    def get_endpoint_url(self) -> str:
        """
        Creates the URL for the API endpoint to send the request to sumbit a job, according to the kind of job that is being created.
    ...
    def get_job_id(self, source: dict) -> str:
        """
        Obtains the job_id from the source parameter. The job_id is defined diferently according to the type of job.
    ...
    def set_job_data(self, source: dict):
        """
        Obtains the data for the job, based on the response from the API
    ...
    def get_job_results(self) -> bool:
        """
        Makes a request to the API using the link of the job to get its status. If the job has been completed, obtains the results of the job.
    ...
    def process_job(self, payload=None) -> bool:
        """
        Submits a new job to be processed, waits until the job is completed and then retrieves the job results.
    ...
    def download_file(self, endpoint_url: str, download_path: str):
        """
        Downloads a file from a job, using the file URL and stores them in download_path
    ...
    def download_job_files(self, download_path = None):
        """
        Downloads all the files from a job ans stores them in the given download_path. If no download path is given, the files are stored in a folder with the name of the job_id.
